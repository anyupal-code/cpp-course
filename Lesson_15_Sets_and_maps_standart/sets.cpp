...
Множество — это структура данных, которая хранит уникальные элементы (без дубликатов) в неупорядоченном (или отсортированном) виде,
где важен только сам факт наличия элемента, а не его порядок или связанное с ним значение.

Коротко:
Множество - коллекция уникальных объектов, где каждый элемент может встречаться только один раз.

В C++ это: std::set (отсортированное) или std::unordered_set (хэш-таблица).
